...
currentAccount: any;
    ...
    loadAll() {
        this.ruleTypeService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<RuleType[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/rule-type'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/rule-type', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    registerChangeInRuleTypes() {
        this.eventSubscriber = this.eventManager.subscribe('ruleTypeListModification', (response) => this.loadAll());
    }
